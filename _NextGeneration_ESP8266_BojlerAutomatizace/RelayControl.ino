...
void turnOnBoilerHeating()
{
  ...
  if(RELAY_DEBUG) DEBUG_OUTPUT.println("RELAY_DEBUG: heatingBoilerRelayState_global: " + (String)((heatingBoilerRelayState_global == ON_RELAY_STATE) ? "ON_RELAY_STATE" : "OFF_RELAY_STATE"));
  if(digitalRead(BOILER_HEATING_RELAY_PIN) != OFF_RELAY_STATE)
  {
    if(RELAY_DEBUG) DEBUG_OUTPUT.println("---->RELAY_DEBUG: turnOnBoilerHeating at temp: " + (String)lastTemp_global);
...
void turnOffBoilerHeating()
{
  ...
  if(RELAY_DEBUG) DEBUG_OUTPUT.println("RELAY_DEBUG: heatingBoilerRelayState_global: " + (String)((heatingBoilerRelayState_global == OFF_RELAY_STATE) ? "OFF_RELAY_STATE" : "ON_RELAY_STATE"));

  if(digitalRead(BOILER_HEATING_RELAY_PIN) != ON_RELAY_STATE)
  {
    if(RELAY_DEBUG) DEBUG_OUTPUT.println("---->RELAY_DEBUG: turnOffBoilerHeating at temp: " + (String)lastTemp_global);
...
{
  if(MAIN_DEBUG) DEBUG_OUTPUT.println("F:cutOffBoilerElectricity()");

  if(digitalRead(BOILER_ELECTRICITY_RELAY_PIN) != OFF_RELAY_STATE)
  {
    if(RELAY_DEBUG) DEBUG_OUTPUT.println("---->RELAY_DEBUG:: cutOffBoilerElectricity");
...
{
  if(MAIN_DEBUG) DEBUG_OUTPUT.println("F:turnOnBoilerElectricity()");

  if(digitalRead(BOILER_ELECTRICITY_RELAY_PIN) != ON_RELAY_STATE)
  {
    if(RELAY_DEBUG) DEBUG_OUTPUT.println("---->RELAY_DEBUG:: turnOnBoilerElectricity");
    //digitalWrite(BOILER_HEATING_RELAY_PIN, HIGH);//turns off normal relay, boiler termostat can still turn on heating - clasical driving
